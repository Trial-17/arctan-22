...
def analyser_et_visualiser_silence_amélioré(
    ...
    try:
        ...
        raise FileNotFoundError(f"Audio file not found at path: {input_path}")
    except Exception as e:
        raise RuntimeError(f"Error loading audio file: {str(e)}")

    # Audio processing - silence detection and segmentation
    try:
        ...
        raise RuntimeError(f"Error during silence detection and analysis: {str(e)}")
    ...
    try:
        ...
        raise RuntimeError(f"Error generating preview visualization: {str(e)}")